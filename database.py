...
class DatabaseManager:
    ...
    async def seed_demo_data_for_user(self, profile_id: str, user_name: str) -> dict:
        """Create sample consultations and prescriptions for a user so the pages
        aren't empty on first load. Safe to call multiple times (checks existing rows)."""
        from datetime import timezone, timedelta
        import json

        # 1. Resolve or create profiles + patient record
        try:
            patient_id = self._ensure_profile_and_patient(
                profile_id, name=user_name
            )
        except Exception as e:
            logger.warning(f"seed_demo_data: patient resolve failed: {e}")
        ...
        if existing_rx.data and existing_cs.data:
            return {"seeded": False, "reason": "demo data already exists"}

        # 3. Seed prescriptions (no doctor FK dependency — doctor_id left null)
        if not existing_rx.data:
            import uuid as _uuid
            rx1_id = str(_uuid.uuid4())
            rx2_id = str(_uuid.uuid4())
            rx_samples = [
                {
                    "id": rx1_id,
                    "patient_id": patient_id,
                    "prescription_number": f"RX-{now.strftime('%Y%m%d')}-001",
                    "status": "approved",
                    "prescribed_by_doctor_id": None,
                    "general_instructions": json.dumps(["Take with water", "Avoid alcohol"]),
                    "dietary_advice": json.dumps(["Light meals", "Avoid spicy food"]),
                    "warning_signs": json.dumps(["Nausea", "Dizziness"]),
                    "follow_up_instructions": "Follow up after 7 days if symptoms persist.",
                    "validity_days": 30,
                    "prescribed_at": (now - timedelta(days=10)).isoformat(),
                    "approved_at": (now - timedelta(days=9)).isoformat(),
                    "expires_at": (now + timedelta(days=20)).isoformat(),
                },
                {
                    "id": rx2_id,
                    "patient_id": patient_id,
                    "prescription_number": f"RX-{now.strftime('%Y%m%d')}-002",
                    "status": "approved",
                    "prescribed_by_doctor_id": None,
                    "general_instructions": json.dumps(["Before food", "Full course of 5 days"]),
                    "dietary_advice": json.dumps(["Avoid dairy while on antibiotics"]),
                    "warning_signs": json.dumps(["Rash", "Difficulty breathing"]),
                    "follow_up_instructions": "Complete full 5-day course.",
                    "validity_days": 30,
                    "prescribed_at": (now - timedelta(days=25)).isoformat(),
                    "approved_at": (now - timedelta(days=24)).isoformat(),
                    "expires_at": (now + timedelta(days=5)).isoformat(),
                },
            ]
            rx_items = [
                {"prescription_id": rx1_id, "medicine_name": "Paracetamol 500mg", "generic_name": "Paracetamol", "dosage": "1 tablet", "frequency": "Twice daily after food", "duration": "5 days", "before_food": False, "cost_estimate": 25},
                {"prescription_id": rx1_id, "medicine_name": "Omeprazole 20mg", "generic_name": "Omeprazole", "dosage": "1 capsule", "frequency": "Once daily before breakfast", "duration": "14 days", "before_food": True, "cost_estimate": 45},
                {"prescription_id": rx2_id, "medicine_name": "Azithromycin 500mg", "generic_name": "Azithromycin", "dosage": "1 tablet", "frequency": "Once daily", "duration": "5 days", "before_food": False, "cost_estimate": 120},
                {"prescription_id": rx2_id, "medicine_name": "Cetirizine 10mg", "generic_name": "Cetirizine", "dosage": "1 tablet", "frequency": "Once at night", "duration": "7 days", "before_food": False, "cost_estimate": 30},
            ]
            try:
                for rx in rx_samples:
                    self.client.table("prescriptions").insert(rx).execute()
                for item in rx_items:
                    self.client.table("prescription_items").insert(item).execute()
            except Exception as e:
                logger.warning(f"seed_demo_data: prescription insert failed: {e}")

        # 4. Seed consultations (use proper UUIDs; doctor_id nullable — no FK dependency)
        if not existing_cs.data:
            import uuid as _uuid
            cs_samples = [
                {
                    "id": str(_uuid.uuid4()),
                    ...
                    "patient_note": "Fever and sore throat for 3 days",
                    "scheduled_at": (now - timedelta(days=10)).isoformat(),
                    "started_at":   (now - timedelta(days=10) + timedelta(hours=1)).isoformat(),
                    "completed_at": (now - timedelta(days=10) + timedelta(hours=2)).isoformat(),
                    "duration_minutes": 20,
                    "summary": "Diagnosed with viral pharyngitis. Prescribed Azithromycin course and rest.",
                    "follow_up_plan": "Follow up in 7 days if fever persists.",
                },
                {
                    "id": str(_uuid.uuid4()),
                    "patient_id": patient_id,
                    ...
                    "status": "scheduled",
                    "specialty": "General Physician",
                    "patient_note": "Acidity and stomach discomfort since morning",
                    "scheduled_at": (now + timedelta(hours=2)).isoformat(),
                },
            ]
            try:
                for cs in cs_samples:
                    self.client.table("consultations").insert(cs).execute()
            except Exception as e:
                logger.warning(f"seed_demo_data: consultation insert failed: {e}")

        return {"seeded": True, "patient_id": patient_id}
